...
const INVOICES_DIR = path.join(ROOT, "data", "invoices");
const TEMPLATE_FILE = path.join(ROOT, "data", "sablona.txt");
...
function invoiceFilePath(id) {
  ...
  return path.join(INVOICES_DIR, `${safe}.txt`);
}
...
async function listInvoices() {
  ...
  for (const file of files) {
    ...
    try {
      const content = await fs.readFile(path.join(INVOICES_DIR, file), "utf-8");
...
async function saveTemplateRecord(template) {
  ...
  await fs.writeFile(TEMPLATE_FILE, templateToTxtContent(record), "utf-8");
  return record;
}
...
ensureDataDirs().then(() => {
  server.listen(PORT, () => {
    ...
    console.log(`Faktury se ukládají do: ${INVOICES_DIR}`);
  });
});
